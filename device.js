...
function getConfigOpts(credentials) {
	if(USE_MQTT) {
		return {
			region: "us-east-1",
            clientId: "use something meaningful here, uniquely identifying this connection - device",
...
function update(thingShadows, thingId, json) {
	return new Promise(function(resolve, reject) {
		...
		console.log("Updating: ", JSON.stringify(json));
...
iot.createThing({thingName: thingId}, function(err, data) {
  if(!err) {
  	init(AWS.config.credentials)
  		...
  		.then(thingShadows => thingShadows.publish("custom_topic", "test message"));
  } else {
  	console.error(err);
  }
})
